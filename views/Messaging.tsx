        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
           ...
           >
              ...
              <div className="text-left flex-1 overflow-hidden">
                <p className="font-bold text-slate-800 dark:text-white truncate">{user.role === UserRole.STUDENT ? MOCK_TEACHER.name : MOCK_STUDENT.name}</p>
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-[2.5rem] border border-neutral-200 dark:border-slate-700 shadow-sm flex flex-col overflow-hidden relative">
        ...
        <div className="p-6 border-b border-neutral-100 dark:border-slate-700 flex items-center gap-4">
           ...
           <div>
              ...
              <p className="text-xs text-emerald-500 font-bold uppercase tracking-widest">Sedang Aktif</p>
           </div>
             ...
             chatMessages.map(msg => (
               <div key={msg.id} className={`flex ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-4 rounded-3xl ${
                    ...
                    <p className={`text-[10px] mt-2 font-bold opacity-60 ${msg.senderId === user.id ? 'text-right' : 'text-left'}`}>
